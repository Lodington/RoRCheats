...
namespace RoRCheats
{
    internal class AdvancedToolTip
    {
        public static RoR2.CharacterBody Body;
        public static RoR2.Inventory Inventory;

        public static void Advancedtooltip()
        {
            var itemDefs = typeof(ItemCatalog).GetFieldValue<ItemDef[]>("itemDefs");
            ...
            foreach (var hud in huds)
            {
                ...
                {
                    var def = RoR2.ItemCatalog.GetItemDef(item.GetFieldValue<RoR2.ItemIndex>("itemIndex"));
                    var index = item.GetFieldValue<RoR2.ItemIndex>("itemIndex");
                    var count = item.GetFieldValue<Int32>("itemCount");
                    item.tooltipProvider.overrideBodyText = Language.GetString(def.descriptionToken) + GetExtraData(index, count);
                }
            }
        }

        private static Double Clover(Double orig, Int32 cloverCount)
        {
            return (1f - Math.Pow((1f - orig / 100.0f), cloverCount + 1)) * 100.0f;
        }

        private static String GetExtraData(RoR2.ItemIndex index, Int32 count)
        {
